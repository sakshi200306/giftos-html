...
$cart_query = "SELECT product_tbl.*, c.cart_qty, c.cart_id 
              FROM card_tbl c
              JOIN product_tbl  ON c.cart_product_id = product_tbl.id
...
while ($item = mysqli_fetch_assoc($cart_result)) {
    $subtotal = $item['MRP'] * $item['cart_qty'];
    $total += $subtotal;
    $cart_items[] = $item;
}
    ...
    try {
        ...
        foreach ($cart_items as $item) {
            $item_query = "INSERT INTO order_items (
                            order_id, id, quantity, MRP, name
                          ) VALUES (
                            '$order_id', '{$item['id']}', '{$item['cart_qty']}', 
                            '{$item['MRP']}', '{$item['name']}'
                          )";
        ...
        $clear_cart = "DELETE FROM card_tbl WHERE cart_customer_id = '$customer_id'";
                    ...
                    <ul class="list-group mb-3">
                        <?php foreach ($cart_items as $item): ?>
                            <li class="list-group-item d-flex justify-content-between lh-sm">
                                ...
                                <span class="text-muted">Rs.<?php echo number_format($item['MRP'] * $item['cart_qty'], 2); ?></span>
                            </li>
                        <?php endforeach; ?>
